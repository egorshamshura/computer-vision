...
EPOCHS = 10
SAMPLE_INTERVAL = 500
OUTPUT_DIR = "output_unconditional"
...
class UnconditionalCelebADataset(Dataset):
    def __init__(self, image_dir, partition_csv, split='train', transform=None):
        self.image_dir = image_dir
        self.transform = transform

        partition = pd.read_csv(partition_csv)
        split_map = {'train': 0, 'val': 1, 'test': 2}
        self.df = partition[partition['partition'] == split_map[split]].reset_index(drop=True)
...
def get_dataloader(split='train'):
    ...
    dataset = UnconditionalCelebADataset(IMAGE_DIR, PARTITION_CSV, split=split, transform=transform)
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        for i, imgs in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}")):
            ...
            if step % 100 == 0:
                print(f"[Epoch {epoch+1}/{EPOCHS}] [Batch {i}/{len(train_loader)}] "
                      ...
                      f"D(real): {d_real.mean().item():.4f}, D(fake): {d_fake.mean().item():.4f}")
